...
import { JwtType } from './rms.types';
import { db } from '@app/database';
import {
    OrderSide,
    UserInvestmentSegment,
    AccountStatusValidationResult,
    SuspensionDetails,
    KycDetails,
    SegmentValidationResult,
    RiskProfileResponse,
    OrderTypeValidationResult,
    ProductOrderTypeCombination,
    QuantityValidationResult,
    ValidityValidationResult,
    LotSizeConfig,
    LotSizeConfiguration,
    MarginValidationResult,
    CheckMarginRequest,
    CashCollateralValidationResult,
    PositionLimitValidationResult,
    PositionLimitCheckData,
    MtmLossLimitResult,
    MtmLossLimitCheck,
    NetPositionRestrictionResult,
    NetPositionCheckData,
...
const checkClientAccountStatus = async (req: Request<JwtType>, res: Response): Promise<void> => {
...
): Promise<AccountStatusValidationResult> => {
        ...
        case AccountStatus.SUSPENDED:
            ...
            return {
                ...
                additionalInfo: {
                    ...
                    requiredActions: [suspensionDetails.requiredAction, 'Contact customer support for assistance'],
                },
            };
...
const fetchPendingKycItems = async (clientId: string): Promise<KycDetails> => {
    const signupCheckpoint = await db
        ...
        .select(['pan_id', 'aadhaar_id', 'address_id', 'ipv'])
        .executeTakeFirst();
...
const updateClientAccountStatus = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();
...
const checkSegmentActivation = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();
...
const determineSegmentFromTradeType = (tradeType: string, symbol: string): UserInvestmentSegment => {
    ...
    switch (tradeType) {
        ...
        default:
            ...
            if (
                uppercaseSymbol.startsWith('GOLD') ||
            ...
            if (
                uppercaseSymbol.startsWith('USDINR') ||
            ...
            if (
                uppercaseSymbol.startsWith('GOVT') ||
...
const getUserRiskProfile = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    ...
    const userProfile = await db
        .selectFrom('user')
        .where('id', '=', userId)
        .select(['annual_income', 'trading_exp', 'occupation', 'marital_status', 'is_politically_exposed'])
        .executeTakeFirst();
    ...
    const response: RiskProfileResponse = {
        ...
        riskScore: riskScore as unknown as RiskScoreResult,
        timestamp: new Date(),
    };
...
const validateOrderType = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();
    ...
    const orderTypeResult = await validateAllowedOrderType(order_type, product_type, trigger_price, segment);

    // If order type validation fails, return early
    if (!orderTypeResult.isValid) {
        await orderTypeValidationCheck(
            clientId,
            {
    ...
    const priceTriggerResult = await validatePriceTriggerRelationship(order_type, order_side, price, trigger_price);
...
const isOrderTypeValidForSegment = async (segment: UserInvestmentSegment, orderType: string): Promise<boolean> => {
...
const validateOrderValidity = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { product_type, validity_type, validity_minutes, symbol, trade_type } = req.body;
    ...
    // order-validity
    const result = await validateValidityType(product_type, validity_type as OrderValidity, validity_minutes, segment);

    // Log the validation result
    await validityValidationCheck(
        clientId,
        {
            productType: product_type,
...
const validateValidityType = async (
    ...
    if (validityType === OrderValidity.MINUTES) {
        if (!validityMinutes || validityMinutes <= 0 || validityMinutes > 180) {
            // Max 3 hours
            return {
                isValid: false,
...
const validateQuantityMultiple = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();
...
const validateEquityQuantity = async (
    ...
    try {
        ...
        const maxQuantity = equityLimit?.max_order_quantity || (tradeType === 'equity_delivery' ? 10000 : 50000); // Example defaults
...
const validateProductOrderCompatibility = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();
    ...
    // Validation Log
    await productOrderCompatibilityCheck(
        clientId,
        {
            productType: product_type,
            orderType: order_type,
...
const fetchStockAttributes = async (
    symbol: string,
    exchange: string,
): Promise<{ isT2TStock: boolean; isIlliquidStock: boolean }> => {
...
// Fetch MTF status
const fetchMTFApproval = async (
    productType: string,
    symbol: string,
    exchange: string,
): Promise<{ isMTFApproved: boolean }> => {
    if (productType !== 'mtf') {
...
const checkAvailableMargin = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { symbol, quantity, price, product_type, order_side, trade_type } = req.body as CheckMarginRequest;
...
const validateAvailableMargin = async (
    ...
    const userFunds = await db
        ...
        .select(['total_funds', 'available_funds', 'blocked_funds', 'used_funds'])
        .executeTakeFirstOrThrow();

    const availableMargin = userFunds.available_funds;
    const marginInfo = await calculateRequiredMargin(symbol, quantity, price, productType, segment);
...
const fetchDerivativeMarginPercentage = async (
    ...
    const productTypeDefaults: Record<string, number> = {
        ...
        options: 0.2, // 20% for options (typically higher than futures)
        intraday: 0.1, // 10% for intraday derivative trades
    };
    ...
    const defaultMargins: Record<UserInvestmentSegment, number> = {
        ...
        Commodity: 0.1, // 10% for Commodity
        Cash: 0.2, // 20% for Cash (should not reach here normally)
        Debt: 0.05, // 5% for Debt
    };

    return defaultMargins[segment] || 0.2;
};

// Fetch margin percentage for equity products
const fetchMarginPercentage = async (productType: string, symbol: string): Promise<number> => {
...
const validateCashCollateralRatio = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { symbol, quantity, price, product_type, trade_type } = req.body;
    ...
    const result = await validateCashCollateral(parseInt(clientId, 10), symbol, quantity, price, product_type, segment);

    // Logging
    await cashCollateralValidationCheck(
        clientId,
        {
            symbol,
            quantity,
...
const checkPositionLimits = async (req: Request<JwtType>, res: Response) => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { symbol, quantity, order_side, trade_type, exchange } = req.body;
    ...
    const lotSize = await getLotSize(symbol, segment, exchange);
    const orderLots = Math.ceil(quantity / lotSize);

    // Validate position limits
    const result = await validatePositionLimits(parseInt(clientId, 10), symbol, orderLots, segment);

    // Log the position limit check
    await positionLimitCheck(
        clientId,
        {
            symbol,
            quantity,
...
const getCurrentPositions = async (userId: number, symbol: string, segment: UserInvestmentSegment) => {
...
const getLotSize = async (symbol: string, segment: UserInvestmentSegment, exchange: string) => {
...
// MTM-loss Function
const checkMtmLossLimit = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { symbol, quantity, price, product_type, order_side, trade_type } = req.body;
...
const isSquareOffOrder = async (
    ...
    if ((netPosition > 0 && orderSide === 'sell') || (netPosition < 0 && orderSide === 'buy')) {
        // Further check if quantity doesn't exceed the position size
        if (
            (orderSide === 'sell' && quantity <= netPosition) ||
...
const updateUserMtmLoss = async (userId: number, realizedLoss: number, unrealizedLoss: number): Promise<void> => {
    // Get current trading date
    const today = new Date();
    today.setHours(0, 0, 0, 0);
...
const getUserMtmLossStatus = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    ...
    res.status(OK).json({
        ...
        remainingCapacity: mtmConfig?.mtm_loss_limit
            ...
            : null,
        status:
            !mtmConfig?.is_active || !mtmConfig?.mtm_loss_limit
...
const checkNetBuySellRestrictions = async (req: Request<JwtType>, res: Response): Promise<void> => {
    const { userId } = req.auth!!;
    const clientId = userId.toString();

    const { symbol, quantity, price, order_side, trade_type, exchange } = req.body;
    ...
    // Log the validation result
    await netPositionRestrictionCheck(
        clientId,
        {
            symbol,
            quantity,
...
const validateNetBuySellRestrictions = async (
    ...
    switch (restriction.restriction_type) {
        case 'TIME_BASED':
            return validateTimeBasedRestriction(restriction, currentPosition, orderSide, quantity, isSquareOff);
...
const getInstrumentRestrictions = async (
    ...
    const restriction = (await db
        ...
        .where((eb) => eb.or([eb('effective_to', 'is', null), eb('effective_to', '>=', currentTime)]))
        .select(['restriction_type', 'cutoff_time', 'max_net_buy_value', 'max_net_position'])
...
const getUserNetPosition = async (userId: number, symbol: string): Promise<number> => {
    const positions = await db
        .selectFrom('trading_positions')
        .where('user_id', '=', userId)
        .where('symbol', '=', symbol)
...
const checkIsSquareOffOrderByPosition = (currentPosition: number, orderSide: OrderSide, quantity: number): boolean => {
...
const getDailyNetBuyValue = async (userId: number, symbol: string): Promise<number> => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
