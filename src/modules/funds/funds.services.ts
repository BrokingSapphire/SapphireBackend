...
export class FundsService {
   ...
   */
  async withdrawFunds(userId: number, amount: number, bankAccountId: number, remarks?: string): Promise<FundTransaction> {
    const result = await db.transaction().execute(async (trx) => {
      // Get user funds
  ...
  async processWithdrawal(
    ...
    const [userFunds, userMargin, foPositions] = await Promise.all([
      ...
      db.selectFrom('user_margin')
        .where('user_id', '=', userId)
        .selectAll()
        .executeTakeFirst(),
        
      db.selectFrom('trading_positions')
        .where('user_id', '=', userId)
    ...
    if (foPositions.length > 0) {
      ...
      const totalPnL = foPositions.reduce((sum, position) => 
        sum + Number(position.mtm_pnl || 0), 0);
  ...
  async completeWithdrawal(transactionId: number): Promise<FundTransaction> {
    ...
    fundsWsHandler.notifyWithdrawalCompleted(
