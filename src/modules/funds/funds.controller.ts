...
export class FundsController {
   ...
   * Get user funds
   */
  async getUserFunds(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async completeFundDeposit(req: Request, res: Response): Promise<void> {
    try {
  ...
  async processWithdrawal(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(200).json({
        ...
        message: `Withdrawal of ${result.safetyCut?.finalAmount} scheduled for processing on ${
          result.scheduled_processing_time?.toLocaleDateString()
        } at ${result.processing_window === 'NOON' ? '12:00 PM' : '6:00 PM'}${
