...
import { OrderSide, ProductType } from '../orders/order.types';

import { ChargesSegmentType, ChargesDirection, ChargeType, ExchangeType } from './charges.types';
import { ChargeCalculationResult, EquityDeliveryCharges, CurrencyCharges } from '@app/database/db';
...
const getExchangeChargePercentage = (
    ...
    if (segment === ChargesSegmentType.CURRENCY_FUTURES) {
        return direction === ChargesDirection.BUY_SELL ? (exchange === ExchangeType.NSE ? 0.00035 : 0.00045) : 0;
    } else {
    ...
    try {
        const chargeTypes = await db
            ...
            .where(
                'name',
                'in',
                appliedCharges.map((charge) => charge.type),
            )
            .select(['id', 'name'])
            .execute();

        const chargeTypeMap = new Map(chargeTypes.map((record) => [record.name, record.id]));
        ...
        await db.transaction().execute(async (trx) => {
            ...
            await trx.updateTable('orders').set({ total_charges: charges.total }).where('id', '=', orderId).execute();

            if (orderChargesValues.length > 0) {
                await trx.insertInto('order_charges').values(orderChargesValues).execute();
            }
        });
    } catch (error) {
...
export const applyOrderCharges = async (orderId: number, userId: number): Promise<ChargeCalculationResult> => {
    // Get order details
